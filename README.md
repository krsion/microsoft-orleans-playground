...
This repository includes a workflow, `.github/workflows/azure-resources.yml`, which provisions these tables. The workflow authenticates using GitHub's OpenID Connect (OIDC) integration, so configure the following secrets and run the workflow:

- `AZURE_CLIENT_ID` – application (client) ID used for OIDC login
